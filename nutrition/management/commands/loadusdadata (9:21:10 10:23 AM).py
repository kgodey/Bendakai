...
class Command(BaseCommand):
	args = 'sr##.ZIP'
	help = 'Load the USDA National Nutrient Database for Standard Reference from the specified zip file.'
	
	def handle(self, path=None, *args, **options):
		if path is None:
			raise CommandError('You must provide a zip file to load the data from.')
		...
		try:
			...
			raise CommandError('An error occurred while trying to read the zip file.')
		else:
			try:
				FOOD_DES = csv.reader(zipfile.open('FOOD_DES.txt', 'r'), 'usda')
				FD_GROUP = csv.reader(zipfile.open('FD_GROUP.txt', 'r'), 'usda')
				NUT_DATA = csv.reader(zipfile.open('NUT_DATA.txt', 'r'), 'usda')
				NUTR_DEF = csv.reader(zipfile.open('NUTR_DEF.txt', 'r'), 'usda')
				SRC_CD = csv.reader(zipfile.open('SRC_CD.txt', 'r'), 'usda')
				DERIV_CD = csv.reader(zipfile.open('DERIV_CD.txt', 'r'), 'usda')
				WEIGHT = csv.reader(zipfile.open('WEIGHT.txt', 'r'), 'usda')
				FOOTNOTE = csv.reader(zipfile.open('FOOTNOTE.txt', 'r'), 'usda')
				DATSRCLN = csv.reader(zipfile.open('DATSRCLN.txt', 'r'), 'usda')
				DATA_SRC = csv.reader(zipfile.open('DATA_SRC.txt', 'r'), 'usda')
			except:
				raise CommandError('An error occurred while trying to read data from the zip file.')
			else:
